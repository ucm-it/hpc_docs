...
# Containers on Pinnacles
Pinnacles **does not** support Docker images or containers due to security reasons. 
Singularity is an open source container platform that is designed for HPC environments. 
## Using Singularity on the Pinnacles Cluster
...
## Building a Container for use on Pinnacles
:::warning
...
1. Execute the following command to load singularity module: `module load singularity`

2. Create a definition file that contains the instructions to build a configure with the proper packages, softwares and other specs. This `.def` file ending denotes a definition file that singularity interprets as an instruction file.
...
2. Start an interactive session and log into the allocated compute node. For more information on running a interactive session view [here](run_jupyter.md)
    ...
    :::
    Because users of Pinnacles do not have sudo access, user must create their containers on Singularity's cloud base for all public containers. Refer to prerequisite material to ensure creation and access to [Sylab Cloud](https://cloud.sylabs.io/dashboard). 
