...
| MPI | MPI ( Message Passing Interface) a standard that describes how information should be passed between nodes and tasks. MPI is a standard, not an implementation. MPI is best used on HPC architectures optimized for parallel computing, such as those with multi-threaded CPUs. Currently the CPUs on Merced and Pinnacles do not have hyperthreading activated. Hyperthreading is a hardware feature that allows a single physical CPU core to present itself to the operating system as two logical cores, letting it run two threads at once by sharing the core's execution resources. This is different from multithreading, which is a software technique for dividing a program into multiple threads and does not by itself require or imply this kind of hardware-level core sharing. |
